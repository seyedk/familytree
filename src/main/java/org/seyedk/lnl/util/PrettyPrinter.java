...
/**
 * This class dumps a family tree to a text file in a format that can be read by
 * a person.
 * 
...
public class PrettyPrinter implements Dumper {
	...
	class PersonSorter implements Comparator<Person> {
		...
		/**
		 * Compares two <code>Person</code>s. If the id of person one is less than the
		 * id of person two, then person one is less than person two. If the id of
		 * person one is greater than the id of person two, then person one greater than
		 * person two. If the id of person one is equal to the id of person two, then
		 * the two persons are equal.
		 */
		@Override
	...
	/**
	 * Creates a new pretty printer that prints to a <code>PrintWriter</code>. This
	 * way, we can print to destinations other than files (such as the console).
	 */
	...
	/**
	 * Creates a new pretty printer that prints to a file of a given name. If the
	...
	public void dump(FamilyTree tree) {
		...
		while (iter.hasNext()) {
			...
			pw.println("Person " + person.getId() + ": " + person.getFullName());
			...
			if (mother != null) {
				pw.println("  Mother: " + mother.getFullName() + " (" + mother.getId() + ")");
			}

			Person father = person.getFather();
			if (father != null) {
				pw.println("  Father: " + father.getFullName() + " (" + father.getId() + ")");
			}
